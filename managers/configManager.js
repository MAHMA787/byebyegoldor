...
export function addPoint(type, args) {
    ...
    let radius = 0.5;
    if (args && !isNaN(args[args.length - 1]) && !["pearlclip", "text"].includes(type)) radius = parseFloat(args[args.length - 1]);
...
register("command", (arg1, arg2, ...args) => {
    if (!arg1) return config().getConfig().openGui();
    ...
    switch (arg1) {
        ...
        case "awaitsecret": {
            ...
            const { config, index, point } = nearestPoint;
            point.awaitsecret = !point.awaitsecret;
            data.save();

            playerUtils.sendMessage(`&7Await Secret: ${point.awaitsecret ? "Enabled" : "Disabled"} for ${point.type}`);
        ...
        case "help": {
            const messages = [
                ...
                `&7/bbg add <start, stop, rotate, bonzo, jump, walk, superboom, align, hclip, swap, blink, etherwarp, pearl, pearlclip, text, impact, tp, lavaclip, airclip, groundwalk>`,
                "&7/bbg radius <radius>",
                "&7/bbg remove"
            ];
...
const packetLogger = register("packetSent", (packet) => {
    if (blinkPoint && !global.inFreeCam && (!playerUtils.isStill() || Player.getY() != Player.getLastY())) {
