...
import { EntityArmorStand, S02PacketChat, S17PacketEntityLookMove } from "../utils/mappings";
...
const actionHandler = register("tick", () => {
	for (let o of Object.entries(entities)) {
        ...
        const readableProgress = ((progress * v.ticks / 20).toFixed(2) * 1000);
        const attackTime = config().autoBloodKillTime == 0 ? playerUtils.ping : config().autoBloodKillTime;

		if (watcherPhase == 1 && readableProgress <= attackTime * 1.5 && !attacking) {
			const rotations = rotationUtils.getRotations(v.final[0], v.final[1] + 2 - config().autoBloodYOffset, v.final[2]);
			rotationUtils.rotate(...rotations);
		}

        if (watcherPhase == 1 && readableProgress <= attackTime) {
			attacking = true;
			const rotations = rotationUtils.getRotations(v.final[0], v.final[1] + 1.5, v.final[2]);
            playerUtils.useItem(config().autoBloodWeapon, rotations, false, false, playerUtils.leftClick());
			attacking = false;
        }
...
const moveTrigger = register("packetReceived", (packet) => {
	...
	if (!textures || !skins.includes(textures)) return;
...
const chatListener = register("packetReceived", (packet) => {
    const message = ChatLib.removeFormatting(packet.func_148915_c().func_150260_c());

	if (message.startsWith("[BOSS] The Watcher:") && waiting) {
		waiting = false;
		watcherPhase = 1;
	}

    if (watcherMessages.includes(message)) watcherPhase = 0;
    else if (message === "[BOSS] The Watcher: Let's see how you can handle this.") waiting = true;
}).setFilteredClass(S02PacketChat).unregister();

const roomListener = register("tick", () => {
    const roomName = dungeonUtils.getRoomName();

    if (roomName === "Blood") {
        if (inRoom) return;

        playerUtils.sendMessage(`&7Entered Blood`);
...
register("step", () => {
	if (dungeonUtils.inDungeon() && config().autoBlood) {
		if (inDungeon) return;

        chatListener.register();
        roomListener.register();
